...
/**
 * Resolve Static Forms key at runtime (bracket access avoids build-time inlining).
 * Static Forms accepts either `apiKey` or `accessKey` — missing both yields:
 * "API key is required (provide either apiKey or accessKey)".
 */
function staticFormsKey(kind = "internal") {
  const raw =
    kind === "customer"
      ? process.env["STATIC_FORMS_CUSTOMER_API_KEY"] ||
        process.env["STATIC_FORMS_API_KEY"]
      : process.env["STATIC_FORMS_API_KEY"];
  const key = normalizeStaticFormsEnvKey(raw);
  if (!key) {
    const err = new Error("STATIC_FORMS_API_KEY is not configured");
    logStaticFormsFailure(`staticFormsKey(${kind})`, err);
    throw err;
  }
  return key;
}
...
function applyStaticFormsKey(form, apiKey) {
  const key = normalizeStaticFormsEnvKey(apiKey || staticFormsKey());
  if (!key) {
    const err = new Error("STATIC_FORMS_API_KEY is not configured");
    logStaticFormsFailure("applyStaticFormsKey", err);
    throw err;
  }
  // Send both field names — Static Forms documents either as valid.
  form.set("apiKey", key);
...
async function postStaticForms(form, apiKey) {
  applyStaticFormsKey(form, apiKey);
  ...
  if (!response.ok || result.success === false) {
    ...
    logStaticFormsFailure(`postStaticForms http=${response.status}`, err);
    throw err;
  }
  return result;
}
...
async function sendCustomerConfirmation({
  ...
  try {
    ...
    logStaticFormsFailure("sendCustomerConfirmation", error);
