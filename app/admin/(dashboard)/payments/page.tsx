...
export default async function PaymentsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Payments"
        description="Live payment rows from Supabase. Checkout creates a pending row; Stripe webhooks mark it paid. Delete any individual payment after confirming."
      />
              ...
              >
                ...
                <div className="mt-4">
                  <DeleteRecordButton
                    ...
                    detail={`${labelStatus(row.payment_type)} · ${labelStatus(row.status)}${row.description ? ` · ${row.description}` : ""}`}
                  />
                </div>
              </article>
            ))}
