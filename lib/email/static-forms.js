...
export async function sendQuotePaymentLinkEmails({
  ...
  const internalSubject = `[QUOTE PAYMENT LINK SENT] — ${label} — ${service}`;
  const internalBody = [
    "A custom Stripe payment link was sent to the customer for a quote.",
    "",
    ...
    "",
    "Payment status will update only after Stripe confirms payment via webhook.",
  ].join("\n");
  ...
  await postStaticForms(
    baseForm({
      ...
      message: internalBody,
      replyTo: email,
      extra: { requestType: "quote_payment_link_sent", service },
    })
  );

  const customerSubject = `Your quote is ready — pay securely (${amount} ${cur})`;
  const customerBody = [
    `Hi ${contactName || "there"},`,
    "",
    "Your custom quote from 4Ward Web Design is ready.",
    "",
    `Service: ${service}`,
    `Amount due: ${amount} ${cur}`,
    "",
    "Pay securely through Stripe using this link:",
    paymentUrl,
    "",
    "This link is for your quoted work only. Opening the page does not charge your card — payment is confirmed only after you complete checkout.",
    "",
    "Questions? Reply to this email or contact us at 4wardwebdesign.com.",
    "",
    "— 4Ward Web Design, LLC",
    "Carlsbad, New Mexico",
  ].join("\n");
...
export async function sendQuotePaymentReceivedEmails({
  ...
  const internalSubject = `[QUOTE PAYMENT RECEIVED] — ${label} — ${service}`;
  const internalBody = [
    ...
    `Checkout session: ${session?.id || "—"}`,
    "",
    "The quote record should now be marked paid in Supabase.",
  ].join("\n");
  ...
  const customerSubject = "Payment received — thank you from 4Ward Web Design";
  const customerBody = [
    `Hi ${contactName || "there"},`,
    "",
    "Thank you! Stripe has confirmed your payment for the quoted work with 4Ward Web Design.",
    "",
    `Service: ${service}`,
    `Amount paid: ${amount} ${cur}`,
    "",
    "We'll follow up shortly with next steps for your project.",
    "",
    "— 4Ward Web Design, LLC",
    "Carlsbad, New Mexico",
    "https://4wardwebdesign.com",
